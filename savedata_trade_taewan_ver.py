...
SELL_FACTOR2 = 1
STANDARD = 4000000
LIMIT = 4  # 조각의 개수를 제한해줘야 한다, 조각당 백만원은 있어야 할듯
MONEY = 500000
ACCOUNT_NUM = "8008232911"  # "5325675810"
...
class Kiwoom(QAxWidget):  # 키움증권의 OpenAPI 가 제공하는 메서드를 호출하기 위해서 QAxWidget 클래스의 인스턴스 필요
    ...
    def _receive_real_data(self, sCode, sRealType, sRealData):
        ...
        if sRealType == '주식체결':
            # 데이터 저장 파트
            price = int(self._comm_real_data(sCode, 28))  # 가격
            ...
            price_for_sell = int(self._comm_real_data(sCode, 27))
            # 시간 데이터 변환
            hour = clock.hour
            minute = clock.minute
            # event_loop 종료 기준 설정
            ...
            if self.stock[sCode].own:
                ...
                if sCode in self.buy_list:
                    ...
                    if price_for_sell <= find_sell_price(buy_price, -SELL_FACTOR2):  # 매도 조건2의 위치
                ...
                if self.stock[sCode].price_time[-1][1] == now:
                    self.stock[sCode].price_time[-1][0] = min(self.stock[sCode].price_time[-1][0], price)
                else:
                ...
                # 미리 검증한 조건에 맞는지 확인
                if price >= find_sell_price(low[0], BUY_FACTOR):  # 매수 조건의 인자
                    ...
                    if LIMIT > len(self.buy_list):
                        ...
                        print("매수", sCode, price, find_sell_price(price, SELL_FACTOR1), find_sell_price(price, -SELL_FACTOR2), clock.minute)
...
if __name__ == "__main__":
    ...
    method3_test(kiwoom.today, code_list)
    print("method test 끝")
    print('프로그램 종료')
# 파이썬 로거 사용해보기
# 파는걸 어떻게 처리할지가 고민, 거래량이냐 감소 이익이냐 등등을 고려해야함, 너무 오래 가지고 있는 경우도 다반사
# VI발동/해제 조건을 사용하면 변동성 완화 종목을 구분할 수 있다.
# 3분간 거래대금이 10억 이상이 아니면 바로 매도하는 형식도 가능할 것 같은데, 돈이 많아야 올라가는 힘이 있지
# not_yet 변수가 왜 0이 되는거지?
# 가끔 매도주문 자체가 안걸리는 경우가 있는데 이유를 모르겠다. 매수 취소는 왜 걸리는지 모르겠음;
# 아 백만원 제한때문에 일정 금액 이하로 내려가도 안먹히는거네;
# 우신시스템, MH에탄올 확인 2021-09-07
# 2021-09-08 price를 현재가가 아니라 최우선 매수호가로 바꿈
# 파는 부분을 시장가가 아니라 지정가로 해줘야 손해를 많이 안볼거같은데, 어떻게 해줘야할지 모르겠음
