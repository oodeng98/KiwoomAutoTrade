...
class KiwoomOpenAPI(QAxWidget):  # 키움증권의 OpenAPI 가 제공하는 메서드를 호출하기 위해서 QAxWidget 클래스의 인스턴스 필요
    def __init__(self):
        super().__init__()
    ...
    def _opw00001(self, rqname, trcode):
        ...
        self.d2_deposit = KiwoomOpenAPI.change_format(d2_deposit)
    ...
    def _opw00018(self, rqname, trcode):
        ...
        self.opw00018_output['single'].append(KiwoomOpenAPI.change_format(total_purchase_price))
        self.opw00018_output['single'].append(KiwoomOpenAPI.change_format(total_eval_price))
        self.opw00018_output['single'].append(KiwoomOpenAPI.change_format(total_eval_profit_loss_price))
        self.opw00018_output['single'].append(KiwoomOpenAPI.change_format(total_earning_rate))
        self.opw00018_output['single'].append(KiwoomOpenAPI.change_format(estimated_deposit))
        ...
        for i in range(rows):
            ...
            quantity = KiwoomOpenAPI.change_format(quantity)
            purchase_price = KiwoomOpenAPI.change_format(purchase_price)
            current_price = KiwoomOpenAPI.change_format(current_price)
            eval_profit_loss_price = KiwoomOpenAPI.change_format(eval_profit_loss_price)
            earning_rate = KiwoomOpenAPI.change_format2(earning_rate)

            self.opw00018_output['multi'].append([name, quantity, purchase_price, current_price,
                                                  eval_profit_loss_price, earning_rate])

        total_earning_rate = KiwoomOpenAPI.change_format(total_earning_rate)
